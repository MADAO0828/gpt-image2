...
function selectedProfile(settings, apiPath = '') { const profiles = Array.isArray(settings.profiles) ? settings.profiles : []; const activeId = settings.activeProfileId || (profiles[0] && profiles[0].id) || 'default-openai'; const cleanPath = String(apiPath || '').replace(/^\/+/, ''); const mode = normalizeAgentMode(settings.agentApiConfigMode); const isResponsesPath = /^responses(?:$|\?|\/)/.test(cleanPath); const agentText = findProfileById(settings, settings.agentTextProfileId); const preferred = isResponsesPath && (mode === 'native' || mode === 'hybrid') && agentText ? agentText : null; const found = preferred || profiles.find(p => p && p.id === activeId) || profiles[0] || null; const base = found || {}; return {
