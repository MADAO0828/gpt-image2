...
async function gk(secret){return crypto.subtle.importKey('raw', new TextEncoder().encode(secret || DEFAULT_SK),{name:'HMAC',hash:'SHA-256'},!1,['verify']);}
async function vt(t, secret){const[h,p,s]=t.split('.');if(!h||!p||!s)throw new Error('invalid');const k=await gk(secret),v=await crypto.subtle.verify('HMAC',k,bd(s),new TextEncoder().encode(h+'.'+p));if(!v)throw new Error('bad');return JSON.parse(new TextDecoder().decode(bd(p)));}
async function vs(r,env){const t=gc(r.headers.get('Cookie')||'','session');if(!t)return null;try{const p=await vt(t, env.JWT_SECRET);if(p.exp&&p.exp<Math.floor(Date.now()/1000))return null;return await env.gpt_image2_db.prepare('SELECT id,username,role FROM users WHERE id=?').bind(p.userId).first();}catch(e){return null;}}
