...
function Invoke-StaticDeployChecks([string]$Url, [string]$Label) {
  ...
  if (-not ($root.Content -match 'home-v3-20260717-image-proxy-parity-r101')) {
    throw "$Label HTML does not contain expected asset version home-v3-20260717-image-proxy-parity-r101."
  }
  ...
  if ($streamRuntime.Content -notmatch 'DEFAULT_SCAN_DEPTH') { throw "$Label image-stream-runtime.js does not contain the expected stack-safe r101 runtime." }
